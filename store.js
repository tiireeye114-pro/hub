const Store = {
    ...
    async init() {
        ...
        if (this.state.students.length < 50 || !this.state.dataVersion || this.state.dataVersion < 9 || isDemo) {
    ...
    seedData() {
        ...
        GRADES.forEach(grade => {
            SECTIONS.forEach(section => {
                for (let i = 0; i < 15; i++) {
                    ...
                    // Free Fee Distribution: F1(4), F2(3), F3(5), F4(4)
                    let isFree = false;
        ...
        if (!this.state.teachers || this.state.teachers.length === 0) {
            this.state.teachers = [
                ...
                { id: 'KT005', name: 'Khalid Ahmed', phone: '+252611223344', gender: 'Male', salary: 100.00, subject: 'History' }
            ];
        }
