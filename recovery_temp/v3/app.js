const App = {
    ...
    showLayout() {
        ...
        if (this.state.currentUserRole === 'teacher') {
            ...
            const attLink = document.querySelector('.nav-item[data-view="attendance"]');
            if (attLink) attLink.style.display = 'flex';

            this.navigateTo('attendance'); // Teachers start at attendance only
        } else {
            // Admin: Show all
    ...
    refreshCurrentView() {
        ...
        if (this.state.currentUserRole === 'teacher' && this.state.currentView !== 'attendance') {
            this.state.currentView = 'attendance';
        }

        const restricted = ['dashboard', 'fees', 'reports', 'data-management'];
        if (restricted.includes(this.state.currentView) && this.state.currentUserRole === 'teacher') {
            // Redundant check but keeps safety
            this.state.currentView = 'attendance';
        }
    ...
    renderExams(container) {
        if (!this.state.currentExamsGrade) {
            this.renderExamsGradeFolders(container);
        } else if (!this.state.currentExamsSection) {
            this.renderExamsSections(container);
        } else if (!this.state.currentExamsSubject) {
            this.renderExamsSubjects(container);
        } else {
            this.renderExamRecording(container);
        }
    },
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 1.5rem;">
                    ${grades.map(grade => `
                        <div onclick="App.openExamsGrade('${grade}')" class="glass-card" style="padding: 2rem; cursor: pointer; transition: all 0.3s ease; text-align: center; border: 1px solid #f3f4f6;">
                            ...
                            <p style="color: #6b7280; font-size: 0.8rem; margin-top: 0.5rem;">Click to view sections</p>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    renderReports(container) {
        ...
        container.innerHTML = `
            <div>
                 <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                    <h3 style="font-size: 1.25rem; font-weight: 700; color: #1f2937;">Reports & Analytics</h3>
